...
public class RatajListRec2 {
	...
	public static int sum(RatajListRec2 head) {
		return head == null ? 0 : head.val + RatajListRec2.sum(head.next);
	}
	...
	public RatajListRec2 delete(int searchedVal) {
		if( val == searchedVal )
			return next;
		else {
			if( next != null )
				next = next.delete(searchedVal);
			// idziemy z nextem w rekurencji po to aby raz zjesc jakas wartosc
			// powracajac za nexta wstawia siê pierwotna wartoœæ i zwracam glowe
			return this;
		}
	}
}
