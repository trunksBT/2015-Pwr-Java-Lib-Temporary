...
public class GraphIndirected2<W,S> implements IGraf<W,S>{
	...
	public S Krawedz(W w1, W w2) {
		if(polaczenia_ != null && w1 != null && w2 != null
				&& wierzcholki_.contains(w1) && wierzcholki_.contains(w2))
		{
			return polaczenia_[indexWierzcholka.get(w1)][indexWierzcholka.get(w2)];
		}
		return null;
	}
	...
	public List<W> Krawedzie(W w) {
		List<W> krawedzie = new LinkedList<W>();
		
		if(w != null && wierzcholki_.contains(w) && indexWierzcholka != null && polaczenia_!=null)
		{
			S[] polaczenia = polaczenia_[indexWierzcholka.get(w)];
			if(polaczenia != null)
			{
				for(int i = 0; i < wierzcholki_.size(); i++)
				{
					if(polaczenia[i] != null)
					{
